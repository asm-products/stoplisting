...
public class LiveFinderItemRejectingTask {
	...
	String mTaskSuccessMsg = "Successfully Rejected Item.";
	String mTaskFailureMsg = "Rejecting Item Failure. Please try again later.";
	String Tag = "LiveFinderTask";

	public LiveFinderItemRejectingTask(Context context, int liveFinderId) {
	...
	TimerTask mTimeTask = new TimerTask() {
		public void run() {
			...
			String strLiveFinderItemRejectingUrl = String.format(
					GlobalDefine.SiteURLs.liveFinderItemRejectingURL,
			...
			try {
				...
				request.setURI(new URI(strLiveFinderItemRejectingUrl));
				...
				try {
					JSONObject itemRejectingResult = new JSONObject(page);
					if (itemRejectingResult != null) {
						String strLoginStatus = itemRejectingResult
								.getString("status");
