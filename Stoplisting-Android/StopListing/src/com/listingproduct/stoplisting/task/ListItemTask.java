...
public class ListItemTask extends AsyncTask<String, Void, String> {
	...
	public ListItemTask(Context context, AnyTaskCallback callback, int userId,
			String itemId) {
		mContext = context;
		mCallback = callback;
		mProgTitle = "Please wait";
		mProgMsg = "Listing Item...";
		...
		nameValuePairs.add(new BasicNameValuePair("field", "list"));
			...
			if (mSuccess) {
				...
				try {
					...
					if (strStatus.equals("200")) {
						strMessage = "Successfully Listed Item.";
					} else {
						strMessage = "Item Listing Failure.";
					}
